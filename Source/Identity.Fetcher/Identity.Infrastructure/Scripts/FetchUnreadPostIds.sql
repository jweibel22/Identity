...
select RowNum, PostId, PostClusterId into #MyTempTable from [ftFetchUnreadPosts](@ChannelId,@UserId,@OrderByColumn,@IsPremium) where RowNum BETWEEN (@FromIndex) AND (@FromIndex+@PageSize) order by RowNum
DECLARE @MaxRow int
